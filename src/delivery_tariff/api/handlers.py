...
from delivery_tariff.api.models import PackageCreate, CreatePackageResponse, ShowPackage, TypePackage
...
async def _get_parcels_by_session_id(page, filter_type_id, filter_price_delivery, db, session_id) -> Union[List[ShowPackage], None]:
    async with db as session:
        async with session.begin():
            package_dal = PackageDAL(session)
...
async def create_package(body: PackageCreate, db: AsyncSession = Depends(get_async_db), session_id = Depends(get_session)) -> CreatePackageResponse:
    return await _create_new_package(body, db, session_id)


@package_router.get("/", response_model=ShowPackage)
async def get_package_by_id(package_id: UUID, db: AsyncSession = Depends(get_async_db), session_id = Depends(get_session)) -> ShowPackage:
...
async def get_parcels_by_session_id(page: int = 1, filter_type_id: Optional[int] = None, filter_price_delivery: Optional[bool] = None, db: AsyncSession = Depends(get_async_db), session_id = Depends(get_session)) -> ShowPackage:
    package = await _get_parcels_by_session_id(page, filter_type_id, filter_price_delivery, db, session_id)
    if package is None:
...
async def get_types_package(db: AsyncSession = Depends(get_async_db)) -> TypePackage:
